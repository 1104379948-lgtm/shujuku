...
### `renderWorldbookForPrompt(scanText, options?)`

按 `shujuku` 正式填表世界书链路渲染当前上下文可见的世界书内容。

**参数**:
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| scanText | string | 否 | 用于触发世界书条目的扫描文本。传空字符串时使用当前世界书管线的默认回退扫描文本。 |
| options | object | 否 | 渲染选项。 |
| options.excludeImportTaggedEntries | boolean | 否 | 是否排除导入标记条目。 |
| options.promptType | string | 否 | 脚本变量 sourceContext 的 promptType，默认 `worldbook`。 |
| options.sourceType | string | 否 | 脚本变量 sourceContext 的 sourceType，默认 `worldbook_api`。 |

**返回值**: `Promise<string>` - 已完成世界书选择、触发、过滤，并经过数据库/SQL/脚本变量正式渲染后的世界书文本；失败或无内容时返回空字符串。

**说明**:
- 该接口复用现有世界书管线，不自行实现条目选择。
- 世界书来源、启用状态、条目触发、屏蔽过滤沿用当前 `shujuku`/角色世界书配置。
- 返回内容会继续走正式世界书变量渲染链路，适合脚本在后台任务中读取“当前扫描文本下模型实际应看到的世界书内容”。

**示例**:
```javascript
const worldbook = await window.AutoCardUpdaterAPI.renderWorldbookForPrompt([
  '当前地点：王城中央广场',
  '本轮剧情：主角听到镖局失踪案的流言。'
].join('\n'));
```
