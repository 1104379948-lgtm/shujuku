...
**标识符兼容与边界**：
- 表名可使用显示名、原始 `CREATE TABLE` 表名或运行时物理表名；列名可使用显示名或运行时列名。
- 显式 DDL 的原始列名恒等于运行时列名；fallback DDL 的原始列名仅在能唯一对应表头时兼容，歧义或多个原始列竞争同一运行时列时保持原文，不按位置猜测。
- 单表裸列与 `a.列名` 形式的 JOIN 限定列会被安全重绑定；多表中无法唯一归属的裸列保持原文，不会猜测。
- 字符串字面量、SQL 注释、CTE 名、派生表别名以及显式或隐式的 SELECT 输出别名不会被重绑定。若本次查询实际引用的别名存在冲突，同样保持原文。
