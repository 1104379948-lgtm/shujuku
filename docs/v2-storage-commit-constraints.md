...
- 仓库层 `writeIsolatedTagData_ACU` 是隔离槽唯一通用写口，带写入屏障：
  候选 `tagData` 若携带 Legacy-V1 表格 payload（`independentData`/`incrementalData`
  含 `sheet_` 键、`modifiedKeys`/`updateGroupKeys` 含 `sheet_` 键、
  `_acu_storage_version === 1` 且存在表字段，含“V2 frame + V1 payload”混合），
  直接抛 `LEGACY_V1_TABLE_WRITE_FORBIDDEN_ACU` 且 message 保持原样（fail-closed）。
- 合法 V2 slot（`storageFrame.version === 2`）与纯向量 metadata
  （`vectorMemoryState`/`summaryVectorIndexState`/`summaryVectorIndexManifest`）不受影响。
- 向量 metadata 更新必须走受控 `patchIsolatedTagDataMetadata_ACU`，禁止 clone 整槽后
  整槽回写（避免旧 V1 payload 被原样续写）；patch 对 V1 payload 槽同样 fail-closed。
- 已删除全部 V1 写能力：`initIsolatedTagSlot_ACU`、`writeTableCheckpointToMessage_ACU`、
  `writeLegacyCompatData_ACU`、`writeLegacyStandardAndSummary_ACU`，
  以及 table-service 的 `persistTablesToChatMessageLegacyV1WithLockOption_ACU`。
- `legacyConfirmed: true` 不再是任何写入授权边界。
- 清理流程不得把 legacy-only 表数据复制到边界楼层/兼容字段；迁移完成前 fail-closed 中止清理。
