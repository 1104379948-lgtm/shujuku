 ...
 */
function buildFallbackColumnNames(headers: (string | null)[]): string[] {
...
function normalizeFallbackHeaderComment(header: string | null, index: number): string {
 ...
 */
function chineseToIdentifier(name: string, fallbackIndex: number): string {
  if (!name) return `col_${fallbackIndex}`;
  // 如果已经是合法标识符，直接返回
  if (/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(name)) return name;
  // 中文名：用下划线连接的 ASCII 化
