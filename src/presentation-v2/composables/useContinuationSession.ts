...
import { readAgentConversation_ACU } from '../../service/continuation/agent/agent-conversation-store';
...
function projectMessage_ACU(message: AgentConversationMessage_ACU): AgentSessionEventInput_ACU {
  ...
  if (message.kind === 'handoff') return { kind: 'thought', title: '早期会话交接报告', detail: message.text };
...
export function useContinuationSession() {
  ...
  function hydrate(): void {
    ...
    try {
      ...
      if (snapshot.messages.length) hydrateAgentSessionLog_ACU(snapshot.messages.map(projectMessage_ACU));
