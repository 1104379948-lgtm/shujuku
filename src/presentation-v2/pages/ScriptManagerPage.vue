<template>
  <section class="acu-v2-script-page">
    ...
    >
      <template #actions>
        <AcuButton v-if="activeResourceTab === 'scripts'" size="sm" @click="triggerImport">
          <i class="fa-solid fa-upload"></i>
          导入脚本包
        </AcuButton>
        <AcuButton v-if="activeResourceTab === 'scripts'" size="sm" :disabled="!scripts.length" @click="exportAllScripts">
          <i class="fa-solid fa-download"></i>
          导出全部
        </AcuButton>
        <AcuButton v-if="activeResourceTab === 'scripts'" size="sm" variant="primary" @click="createScript">
          <i class="fa-solid fa-plus"></i>
          新增脚本
        </AcuButton>
        <AcuButton v-if="activeResourceTab === 'libraries'" size="sm" variant="primary" @click="createLibrary">
          <i class="fa-solid fa-plus"></i>
          新增共享库
        </AcuButton>
      </template>
      ...
      <section v-if="importPreview" class="acu-v2-script-page__import-preview" aria-label="脚本导入预览">
        <div class="acu-v2-script-page__import-head">
          <div>
            <strong>导入预览</strong>
            <p class="acu-v2-script-page__hint">格式 {{ importPreview.format }}，共 {{ importPreview.libraries.length }} 个共享库、{{ importPreview.scripts.length }} 个脚本。</p>
          </div>
      ...
      <div class="acu-v2-script-page__resource-tabs" role="tablist" aria-label="脚本管理资源类型">
        <button type="button" :class="{ 'acu-v2-script-page__resource-tab--active': activeResourceTab === 'scripts' }" @click="activeResourceTab = 'scripts'">脚本</button>
        <button type="button" :class="{ 'acu-v2-script-page__resource-tab--active': activeResourceTab === 'libraries' }" @click="activeResourceTab = 'libraries'">共享库</button>
      </div>

      <div v-if="activeResourceTab === 'scripts'" class="acu-v2-script-page__layout">
        ...
        <main v-if="draft" class="acu-v2-script-page__editor" aria-label="脚本编辑页">
          ...
          <section class="acu-v2-script-page__section">
            ...
            <div v-if="draft.scope.type === 'character'" class="acu-v2-script-page__character-scope">
              <div class="acu-v2-script-page__character-scope-head">
                <strong>角色卡作用域</strong>
                <span class="acu-v2-script-page__hint">已选 {{ selectedCharacterNames.length }} 个</span>
              </div>
              <div v-if="selectedCharacterOptions.length" class="acu-v2-script-page__character-chips">
                <button v-for="option in selectedCharacterOptions" :key="option.name" type="button" class="acu-v2-script-page__character-chip" @click="removeCharacterScope(option.name)">
                  <span>{{ option.label }}</span>
                  <i class="fa-solid fa-xmark" aria-hidden="true"></i>
                </button>
              </div>
              <div v-else class="acu-v2-script-page__empty">尚未选择角色卡。</div>
              <div class="acu-v2-script-page__character-add-row">
                <select :value="''" :disabled="!unselectedCharacterOptions.length" @change="handleAddCharacterSelect($event)">
                  <option value="">添加角色卡...</option>
                  <option v-for="option in unselectedCharacterOptions" :key="option.name" :value="option.name">{{ option.label }}</option>
                </select>
                <AcuButton size="sm" :disabled="!currentCharacterName" @click="bindCurrentCharacter">选中当前角色</AcuButton>
              </div>
          ...
          <section class="acu-v2-script-page__section">
            <h3>依赖共享库</h3>
            <p class="acu-v2-script-page__hint">脚本只能通过 <code>ctx.importLibrary(name)</code> 加载这里显式勾选的共享库。</p>
            <div v-if="!libraries.length" class="acu-v2-script-page__empty">暂无共享库；切换到“共享库”创建公共方法库。</div>
            <div v-else class="acu-v2-script-page__library-picker">
              <label v-for="library in libraries" :key="library.id" class="acu-v2-script-page__library-option">
                <input type="checkbox" :checked="isLibrarySelected(library.name)" @change="toggleLibraryDependency(library.name, ($event.target as HTMLInputElement).checked)" />
                <span>{{ library.name }}</span>
                <small>v{{ library.version }}{{ library.enabled === false ? ' · 已禁用' : '' }}</small>
              </label>
            </div>
          </section>
              ...
              <div class="acu-v2-script-page__binding-head">
                <label class="acu-v2-script-page__binding-field acu-v2-script-page__binding-field--hook">
                  ...
                  <select v-model="binding.hook" @change="handleBindingHookChange(index)">
                    <option v-for="hook in hookOptions" :key="hook" :value="hook">{{ hookOptionLabel(hook) }}</option>
                  </select>
                </label>
          ...
          <section class="acu-v2-script-page__section">
            ...
            <label class="acu-v2-script-page__field">
              ...
              <select v-model="manualBindingIndex">
                ...
                <option v-for="(binding, index) in draft.bindings" :key="`${binding.hook}-${index}`" :value="String(index)">{{ index + 1 }}. {{ hookLabel(binding.hook) }}</option>
              </select>
            </label>
      ...
      <div v-else class="acu-v2-script-page__layout">
        <aside class="acu-v2-script-page__list" aria-label="共享库列表">
          <div v-if="!libraries.length" class="acu-v2-script-page__empty">暂无共享库，点击“新增共享库”开始。</div>
          <button
            v-for="library in libraries"
            :key="library.id"
            type="button"
            class="acu-v2-script-page__script-card"
            :class="{ 'acu-v2-script-page__script-card--active': library.id === selectedLibraryId }"
            @click="selectLibrary(library.id)"
          >
            <span class="acu-v2-script-page__script-title">{{ library.name }}</span>
            <span class="acu-v2-script-page__script-meta">{{ library.enabled ? '启用' : '禁用' }} · v{{ library.version }}</span>
            <span class="acu-v2-script-page__script-meta">{{ library.description || '无描述' }}</span>
            <span class="acu-v2-script-page__script-meta">ID: {{ library.id }}</span>
          </button>
        </aside>

        <main v-if="libraryDraft" class="acu-v2-script-page__editor" aria-label="共享库编辑页">
          <div class="acu-v2-script-page__toolbar">
            <AcuButton variant="primary" @click="saveLibraryDraft">保存共享库</AcuButton>
            <AcuButton @click="duplicateLibraryDraft">复制</AcuButton>
            <AcuButton variant="danger" @click="deleteLibraryDraft">删除</AcuButton>
          </div>
          <div class="acu-v2-script-page__grid">
            <label class="acu-v2-script-page__field">
              <span>库名</span>
              <input v-model="libraryDraft.name" type="text" />
            </label>
            <label class="acu-v2-script-page__field">
              <span>版本</span>
              <input v-model.number="libraryDraft.version" type="number" min="1" step="1" />
            </label>
            <label class="acu-v2-script-page__checkbox">
              <input v-model="libraryDraft.enabled" type="checkbox" />
              <span>启用共享库</span>
            </label>
          </div>
          <label class="acu-v2-script-page__field">
            <span>描述</span>
            <input v-model="libraryDraft.description" type="text" />
          </label>
          <section class="acu-v2-script-page__section">
            <h3>共享库源码</h3>
            <p class="acu-v2-script-page__hint">源码是 ESM 模块。推荐导出 <code>install(ctx)</code>，脚本中通过 <code>const helpers = (await ctx.importLibrary('库名')).install(ctx)</code> 使用。</p>
            <textarea v-model="libraryDraft.source" class="acu-v2-script-page__source acu-v2-script-page__source--library" spellcheck="false"></textarea>
          </section>
          <section class="acu-v2-script-page__section">
            <h3>引用方式</h3>
            <div class="acu-v2-script-page__example-list">
              <div class="acu-v2-script-page__example-item">
                <strong>脚本依赖</strong>
                <code>libraryNames: [{{ JSON.stringify(libraryDraft.name) }}]</code>
              </div>
              <div class="acu-v2-script-page__example-item">
                <strong>脚本源码</strong>
                <code>const helpers = (await ctx.importLibrary({{ JSON.stringify(libraryDraft.name) }})).install(ctx);</code>
              </div>
            </div>
          </section>
        </main>
...
const hookLabels: Record<ScriptHookName_ACU, string> = {
  'chat.loaded': '聊天加载后',
  'db.loaded': '数据库加载后',
  'plot.before_task_request': '剧情任务请求前',
  'plot.after_task_response': '剧情任务响应后',
  'plot.after_stage': '剧情阶段完成后',
  'main_reply.before_generation': '主回复生成前',
  'main_reply.after_response': '主回复响应后',
  'table_fill.before_request': '填表请求前',
  'table_fill.after_commit': '填表写入后',
  'plot_worldbook.before_render': '剧情世界书渲染前',
  'table_fill_worldbook.before_render': '填表世界书渲染前',
  'manual_table_save.after_commit': '手动表格保存后',
};

const scripts = ref<UserScriptDefinition_ACU[]>([]);
const libraries = ref<UserScriptLibrary_ACU[]>([]);
const logs = ref<ScriptLogEntry_ACU[]>([]);
const selectedId = ref('');
const selectedLibraryId = ref('');
const activeResourceTab = ref<'scripts' | 'libraries'>('scripts');
const draft = ref<UserScriptDefinition_ACU | null>(null);
const libraryDraft = ref<UserScriptLibrary_ACU | null>(null);
...
const selectedLogGroups = computed(() => {
  ...
  return Array.from(groups.entries()).map(([runId, groupLogs]) => {
    ...
    return {
      ...
      callLabel: [first?.callType, first?.hook ? hookLabel(first.hook) : ''].filter(Boolean).join(' · ') || 'log',
      lastAt: last?.timestamp || first?.timestamp || 0,
    };
  }).sort((a, b) => b.lastAt - a.lastAt);
});
const selectedCharacterNames = computed(() => (draft.value?.scope.characterNames || []).map(name => String(name).trim()).filter(Boolean));
const characterOptions = computed(() => {
  const options = readAvailableCharacterNames_ACU().map(name => ({ name, label: name }));
  for (const name of selectedCharacterNames.value) {
    if (name && !options.some(option => option.name === name)) {
      options.unshift({ name, label: `${name}（当前列表未找到）` });
    }
  }
  return options;
});
const selectedCharacterOptions = computed(() => selectedCharacterNames.value.map(name => {
  const option = characterOptions.value.find(item => item.name === name);
  return option || { name, label: name };
}));
const unselectedCharacterOptions = computed(() => {
  const selected = new Set(selectedCharacterNames.value);
  return characterOptions.value.filter(option => !selected.has(option.name));
});
...
const importPreviewItems = computed(() => {
  ...
  const hooks = bindings.map(binding => hookLabel(binding?.hook)).filter(Boolean).join(', ') || '无挂载点';
...
function hookLabel(hook: unknown): string {
  const key = String(hook || '') as ScriptHookName_ACU;
  return hookLabels[key] || String(hook || '未知挂载点');
}

function hookOptionLabel(hook: ScriptHookName_ACU): string {
  return `${hookLabel(hook)}（${hook}）`;
}
...
function refresh(): void {
  ...
  libraries.value = getScriptLibraries_ACU().slice().sort((a, b) => a.name.localeCompare(b.name) || a.id.localeCompare(b.id));
  ...
  if (!selectedLibraryId.value && libraries.value[0]) selectedLibraryId.value = libraries.value[0].id;
  if (selectedLibraryId.value && !libraries.value.some(library => library.id === selectedLibraryId.value)) selectedLibraryId.value = libraries.value[0]?.id || '';
  draft.value = selectedId.value ? clone(scripts.value.find(script => script.id === selectedId.value) || null) : null;
  libraryDraft.value = selectedLibraryId.value ? clone(libraries.value.find(library => library.id === selectedLibraryId.value) || null) : null;
...
function readAvailableCharacterNames_ACU(): string[] {
  const names = new Set<string>();
  try {
    const context = (topLevelWindow_ACU as any)?.SillyTavern?.getContext?.();
    const contextCharacters = Array.isArray(context?.characters) ? context.characters : [];
    contextCharacters.forEach((character: any) => {
      const name = String(character?.name || '').trim();
      if (name) names.add(name);
    });
    const windowCharacters = Array.isArray((topLevelWindow_ACU as any)?.characters) ? (topLevelWindow_ACU as any).characters : [];
    windowCharacters.forEach((character: any) => {
      const name = String(character?.name || '').trim();
      if (name) names.add(name);
    });
  } catch (_) {}
...
function createDefaultLibrary(): UserScriptLibrary_ACU {
  const now = Date.now();
  return {
    id: '',
    name: '新共享库',
    ...
    version: 1,
    language: 'javascript',
    source: "export function install(ctx) {\n  return {\n    hello() { return 'hello'; }\n  };\n}\n",
    createdAt: now,
    updatedAt: now,
  };
}
...
function createLibrary(): void {
  try {
    const saved = upsertScriptLibrary_ACU(createDefaultLibrary(), true);
    selectedLibraryId.value = saved.id;
    activeResourceTab.value = 'libraries';
    refresh();
  } catch (error) {
    showScriptManagerError_ACU('新增共享库失败', error);
  }
}
...
function saveLibraryDraft(): UserScriptLibrary_ACU | null {
  if (!libraryDraft.value) return null;
  try {
    const saved = upsertScriptLibrary_ACU(libraryDraft.value, true);
    selectedLibraryId.value = saved.id;
    refresh();
    return saved;
  } catch (error) {
    showScriptManagerError_ACU('保存共享库失败', error);
    return null;
  }
}

function deleteLibraryDraft(): void {
  if (!libraryDraft.value) return;
  if (!confirm(`删除共享库「${libraryDraft.value.name}」？`)) return;
  try {
    deleteScriptLibrary_ACU(libraryDraft.value.id, true);
    selectedLibraryId.value = '';
    refresh();
  } catch (error) {
    showScriptManagerError_ACU('删除共享库失败', error);
  }
}

function duplicateLibraryDraft(): void {
  if (!libraryDraft.value) return;
  const copy = clone(libraryDraft.value);
  copy.id = '';
  copy.name = `${copy.name} 副本`;
  copy.createdAt = Date.now();
  copy.updatedAt = Date.now();
  try {
    const saved = upsertScriptLibrary_ACU(copy, true);
    selectedLibraryId.value = saved.id;
    refresh();
  } catch (error) {
    showScriptManagerError_ACU('复制共享库失败', error);
  }
}

function isLibrarySelected(name: string): boolean {
  return (draft.value?.libraryNames || []).includes(String(name || '').trim());
}

function toggleLibraryDependency(name: string, checked: boolean): void {
  if (!draft.value) return;
  const normalizedName = String(name || '').trim();
  if (!normalizedName) return;
  const names = new Set((draft.value.libraryNames || []).map(item => String(item || '').trim()).filter(Boolean));
  if (checked) names.add(normalizedName);
  else names.delete(normalizedName);
  draft.value.libraryNames = Array.from(names).sort((a, b) => a.localeCompare(b));
}
...
function isCharacterSelected(name: string): boolean {
  return selectedCharacterNames.value.includes(String(name || '').trim());
}

function addCharacterScope(name: string): void {
  toggleCharacterScope(name, true);
}

function handleAddCharacterSelect(event: Event): void {
  const select = event.target as HTMLSelectElement;
  addCharacterScope(select.value);
  select.value = '';
}

function removeCharacterScope(name: string): void {
  toggleCharacterScope(name, false);
}

function toggleCharacterScope(name: string, checked: boolean): void {
  if (!draft.value) return;
  const normalizedName = String(name || '').trim();
  if (!normalizedName) return;
  const names = new Set(selectedCharacterNames.value);
  if (checked) names.add(normalizedName);
  else names.delete(normalizedName);
  draft.value.scope.characterNames = Array.from(names).sort((a, b) => a.localeCompare(b));
}
...
function validateImportPreview(payload: unknown): UserScriptExportPackage_ACU {
  if (!payload || typeof payload !== 'object' || (payload as any).format !== USER_SCRIPT_EXPORT_FORMAT_ACU || !Array.isArray((payload as any).libraries) || !Array.isArray((payload as any).scripts)) {
...
function bindingSummary(script: UserScriptDefinition_ACU): string {
  const hooks = (script.bindings || []).map(binding => hookLabel(binding.hook)).filter(Boolean);
...
function logCallLabel(log: ScriptLogEntry_ACU): string {
  ...
  if (log.hook) parts.push(hookLabel(log.hook));
...
.acu-v2-script-page__character-scope { display: flex; flex-direction: column; gap: 8px; padding: 10px; border: 1px solid var(--acu-border-2); border-radius: var(--acu-radius-sm); background: var(--acu-bg-1); }
.acu-v2-script-page__character-scope-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; color: var(--acu-text-2); }
.acu-v2-script-page__character-chips { display: flex; flex-wrap: wrap; gap: 8px; }
.acu-v2-script-page__character-chip { display: inline-flex; align-items: center; gap: 8px; max-width: 360px; padding: 6px 10px; border: 1px solid var(--acu-border-2); border-radius: 999px; background: var(--acu-bg-2); color: var(--acu-text-1); font: inherit; cursor: pointer; }
.acu-v2-script-page__character-chip span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.acu-v2-script-page__character-chip i { color: var(--acu-text-3); }
.acu-v2-script-page__character-add-row { display: grid; grid-template-columns: minmax(220px, 420px) auto; gap: 8px; align-items: center; }
