...
import { getCombinedWorldbookContent_ACU } from '../../../service/worldbook/pipeline';
import { renderTableFillWorldbookForPrompt_ACU } from '../../../service/ai/prompt-builder/prompt-prepare';
import { getCurrentScriptRequestContext_ACU, type ScriptRequestContext_ACU } from '../../../service/scripts/script-request-context';
...
export function createWorldbookAiApi(_ctx: ApiGroupContext): Record<string, Function> {
    return {
        ...
        // 按 shujuku 正式填表世界书链路渲染世界书内容：世界书选择/触发/过滤 + db/sql/script 变量处理
        renderWorldbookForPrompt: async function(scanText: any = '', options: any = {}) {
            try {
                const normalizedScanText = typeof scanText === 'string' ? scanText : String(scanText || '');
                const rawWorldbookContent = await getCombinedWorldbookContent_ACU(normalizedScanText, {
                    excludeImportTaggedEntries: options?.excludeImportTaggedEntries === true,
                });
                if (!rawWorldbookContent) {
                    return '';
                }
                const currentRequestContext = getCurrentScriptRequestContext_ACU();
                const requestContext: ScriptRequestContext_ACU = options?.requestContext?.requestId
                    ? options.requestContext
                    : {
                        ...currentRequestContext,
                        source: {
                            ...(currentRequestContext.source || {}),
                            promptType: options?.promptType || 'worldbook',
                            sourceType: options?.sourceType || 'worldbook_api',
                        },
                    };
                return await renderTableFillWorldbookForPrompt_ACU(rawWorldbookContent, requestContext);
            } catch (e) {
                logError_ACU('renderWorldbookForPrompt failed:', e);
                return '';
            }
        },
