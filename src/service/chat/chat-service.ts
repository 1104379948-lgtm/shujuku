...
import { loadAllChatMessages_ACU } from '../worldbook/pipeline';
...
function normalizeManualRefillTargetSheetKeys_ACU(targetSheetKeys: unknown): string[] {
    ...
    targetSheetKeys.forEach((value) => {
        ...
        if (!key || !key.startsWith('sheet_') || seen.has(key)) return;
        seen.add(key);
        normalized.push(key);
    });
    return normalized;
...
type ManualRefillDeletionTargetKind_ACU = 'objectKey' | 'arrayValue' | 'stringReference';

interface ManualRefillDeletionTarget_ACU {
    sheetKey: string;
    path: string;
    kind: ManualRefillDeletionTargetKind_ACU;
}

const MAX_MANUAL_REFILL_DELETION_LOG_TARGETS_ACU = 80;

function collectManualRefillTargetKeyPaths_ACU(value: unknown, path: string, sheetKeySet: Set<string>, targets: ManualRefillDeletionTarget_ACU[], depth = 0): void {
    if (targets.length >= MAX_MANUAL_REFILL_DELETION_LOG_TARGETS_ACU || depth > 12) return;
    if (typeof value === 'string') {
        for (const sheetKey of sheetKeySet) {
            if (value.includes(sheetKey)) {
                targets.push({ sheetKey, path, kind: 'stringReference' });
                if (targets.length >= MAX_MANUAL_REFILL_DELETION_LOG_TARGETS_ACU) return;
            }
        }
        return;
    }
...
function summarizeManualRefillDeletionTargets_ACU(msg: any, targetSheetKeys: string[]): ManualRefillDeletionTarget_ACU[] {
    const targets: ManualRefillDeletionTarget_ACU[] = [];
...
function summarizeManualRefillMessageStoragePresence_ACU(msg: any) {
    const isolatedData = msg?.TavernDB_ACU_IsolatedData;
...
function createManualRefillResidueSummary_ACU() {
    return {
        ...
        matchingMessageIndices: [] as number[],
        substringOnlyPaths: [] as string[],
...
function scanManualRefillRangeResidueSummary_ACU(chat: any[], targetMessageIndices: number[], isolationKey: string, targetSheetKeys: string[]) {
    ...
    for (const idx of targetMessageIndices) {
        ...
        if (report.isolationKeyMatched) summary.isolationKeyMatchedMessages++;
        ...
        if (messageHasResidue) summary.matchingMessageIndices.push(idx);
...
async function clearManualRefillSheetDataInRangeCore_ACU(targetMessageIndices: number[], targetSheetKeys: string[] | null = null): Promise<number> {
    ...
    const missedMessages: Array<{ index: number; presence: ReturnType<typeof summarizeManualRefillMessageStoragePresence_ACU> }> = [];
...
export async function clearManualRefillSheetDataInRange_ACU(targetMessageIndices: number[], targetSheetKeys: string[] | null = null): Promise<number> {
    ...
    const writeSet = normalizedTargetSheetKeys.map(sheetKey => ({ kind: 'sheet' as const, sheetKey }));
    return runTableWriteTransaction_ACU({
        source: 'system_cleanup',
