...
import type { ScriptHookName_ACU, ScriptLogEntry_ACU, UserScriptDefinition_ACU, UserScriptLibrary_ACU } from './script-types';
...
export interface UserScriptExportPackage_ACU {
  ...
  libraries: Partial<UserScriptLibrary_ACU>[];
...
function ensureScriptSettings_ACU(): { userScripts: UserScriptDefinition_ACU[]; scriptLibraries: UserScriptLibrary_ACU[]; scriptLogs: ScriptLogEntry_ACU[] } {
  const global = ensureGlobalScriptSettings_ACU();
  settings_ACU.userScripts = global.userScripts;
  settings_ACU.scriptLibraries = global.scriptLibraries;
  settings_ACU.scriptLogs = global.scriptLogs;
  return { userScripts: global.userScripts as UserScriptDefinition_ACU[], scriptLibraries: global.scriptLibraries as UserScriptLibrary_ACU[], scriptLogs: global.scriptLogs as ScriptLogEntry_ACU[] };
}
...
function createScriptLibraryId_ACU(): string {
  return `script_lib_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
}
...
function validateImportScript_ACU(script: unknown, index: number): asserts script is Partial<UserScriptDefinition_ACU> {
  ...
  if (typeof (script as any).libraryNames !== 'undefined') {
    if (!Array.isArray((script as any).libraryNames)) throw new Error(`脚本导入字段无效: ${prefix}.libraryNames`);
    if (!(script as any).libraryNames.every((item: unknown) => typeof item === 'string' && String(item).trim())) throw new Error(`脚本导入字段无效: ${prefix}.libraryNames`);
  }
...
function validateImportLibrary_ACU(library: unknown, index: number): asserts library is Partial<UserScriptLibrary_ACU> {
  const prefix = `libraries[${index}]`;
  if (!isPlainObject_ACU(library)) throw new Error(`脚本库导入字段无效: ${prefix}`);
  assertStringField_ACU((library as any).name, `${prefix}.name`, { required: true });
  assertStringField_ACU((library as any).description, `${prefix}.description`);
  assertBooleanField_ACU((library as any).enabled, `${prefix}.enabled`, { required: true });
  assertFiniteNumberField_ACU((library as any).version, `${prefix}.version`);
  if (typeof (library as any).language === 'undefined') throw new Error(`脚本库导入缺少字段: ${prefix}.language`);
  if ((library as any).language !== 'javascript') throw new Error(`脚本库导入字段无效: ${prefix}.language`);
  assertStringField_ACU((library as any).source, `${prefix}.source`, { required: true });
  assertFiniteNumberField_ACU((library as any).createdAt, `${prefix}.createdAt`);
  assertFiniteNumberField_ACU((library as any).updatedAt, `${prefix}.updatedAt`);
}

function validateLibraryReferences_ACU(scripts: Partial<UserScriptDefinition_ACU>[], libraries: Partial<UserScriptLibrary_ACU>[]): void {
  const libraryNames = new Set<string>();
  for (const library of libraries) {
    const name = String(library.name || '').trim();
    if (libraryNames.has(name)) throw new Error(`脚本库名称重复: ${name}`);
    libraryNames.add(name);
  }
  for (const [scriptIndex, script] of scripts.entries()) {
    const names = script.libraryNames || [];
    const seen = new Set<string>();
    for (const name of names) {
      const normalized = String(name || '').trim();
      if (seen.has(normalized)) throw new Error(`脚本导入字段无效: scripts[${scriptIndex}].libraryNames`);
      seen.add(normalized);
      if (!libraryNames.has(normalized)) throw new Error(`脚本依赖的脚本库不存在: scripts[${scriptIndex}].libraryNames.${normalized}`);
    }
  }
}
...
function createStoredUserScript_ACU(script: Partial<UserScriptDefinition_ACU>, options: { preserveIds?: boolean } = {}): UserScriptDefinition_ACU {
  ...
  return {
    ...
    libraryNames: cloneScriptJson_ACU((script.libraryNames || []).map(name => String(name).trim()).filter(Boolean)),
...
function createStoredScriptLibrary_ACU(library: Partial<UserScriptLibrary_ACU>, options: { preserveIds?: boolean } = {}): UserScriptLibrary_ACU {
  const now = Date.now();
  return {
    id: options.preserveIds && library.id ? String(library.id) : createScriptLibraryId_ACU(),
    name: String(library.name || '').trim(),
    description: library.description,
    enabled: library.enabled === true,
    version: Number.isFinite(library.version) ? Number(library.version) : 1,
    language: 'javascript',
    source: String(library.source || ''),
    createdAt: Number.isFinite(library.createdAt) ? Number(library.createdAt) : now,
    updatedAt: now,
  };
}
...
export function getScriptLibraries_ACU(): UserScriptLibrary_ACU[] {
  return cloneScriptJson_ACU(ensureScriptSettings_ACU().scriptLibraries);
}

export function getScriptLibrariesForRuntime_ACU(): UserScriptLibrary_ACU[] {
  return ensureScriptSettings_ACU().scriptLibraries;
}

export function findScriptLibraryByNameForRuntime_ACU(name: string): UserScriptLibrary_ACU | undefined {
  const normalizedName = String(name || '').trim();
  if (!normalizedName) return undefined;
  return getScriptLibrariesForRuntime_ACU().find(library => library.name === normalizedName);
}
...
export function saveUserScripts_ACU(scripts: UserScriptDefinition_ACU[], persist = true): void {
  ...
  validateLibraryReferences_ACU(scripts, getScriptLibrariesForRuntime_ACU());
...
export function saveScriptLibraries_ACU(libraries: UserScriptLibrary_ACU[], persist = true): void {
  libraries.forEach((library, index) => validateImportLibrary_ACU(library, index));
  const nextLibraries = cloneScriptJson_ACU(libraries);
  validateLibraryReferences_ACU(getUserScriptsForRuntime_ACU(), nextLibraries);
  const state = ensureScriptSettings_ACU();
  state.scriptLibraries.splice(0, state.scriptLibraries.length, ...nextLibraries);
  if (persist) assertSaveResult_ACU(saveSettings_ACU());
}

export function upsertScriptLibrary_ACU(library: UserScriptLibrary_ACU, persist = true): UserScriptLibrary_ACU {
  validateImportLibrary_ACU(library, 0);
  const libraries = [...getScriptLibrariesForRuntime_ACU()];
  const saved = createStoredScriptLibrary_ACU(library, { preserveIds: !!library.id });
  const duplicate = libraries.find(item => item.name === saved.name && item.id !== saved.id);
  if (duplicate) throw new Error(`脚本库名称重复: ${saved.name}`);
  const index = libraries.findIndex(item => item.id === saved.id);
  if (index >= 0) libraries[index] = saved;
  else libraries.push(saved);
  saveScriptLibraries_ACU(libraries, persist);
  return saved;
}

export function deleteScriptLibrary_ACU(libraryId: string, persist = true): boolean {
  const libraries = getScriptLibrariesForRuntime_ACU();
  const library = libraries.find(item => item.id === libraryId);
  if (!library) return false;
  const referencingScript = getUserScriptsForRuntime_ACU().find(script => (script.libraryNames || []).includes(library.name));
  if (referencingScript) throw new Error(`脚本库仍被脚本引用: ${library.name} -> ${referencingScript.name}`);
  const next = libraries.filter(item => item.id !== libraryId);
  saveScriptLibraries_ACU(next, persist);
  return true;
}
...
export function exportUserScripts_ACU(scriptIds?: string[]): UserScriptExportPackage_ACU {
  ...
  const scripts = getUserScripts_ACU()
    ...
    .map(script => ({
      ...
      libraryNames: cloneScriptJson_ACU(script.libraryNames),
  ...
  const requiredLibraryNames = new Set<string>();
  for (const script of getUserScripts_ACU().filter(script => !idSet || idSet.has(script.id))) {
    for (const name of script.libraryNames || []) requiredLibraryNames.add(name);
  }
  const libraries = getScriptLibraries_ACU()
    .filter(library => requiredLibraryNames.has(library.name))
    .map(library => ({
      name: library.name,
      description: library.description,
  ...
  return { format: USER_SCRIPT_EXPORT_FORMAT_ACU, libraries, scripts };
}

export function importUserScripts_ACU(payload: unknown, persist = true): UserScriptDefinition_ACU[] {
  const parsed = typeof payload === 'string' ? JSON.parse(payload) : payload;
  if (!parsed || typeof parsed !== 'object' || (parsed as any).format !== USER_SCRIPT_EXPORT_FORMAT_ACU || !Array.isArray((parsed as any).libraries) || !Array.isArray((parsed as any).scripts)) {
    throw new Error('脚本导入格式无效');
  }
  ((parsed as any).libraries as unknown[]).forEach(validateImportLibrary_ACU);
  ((parsed as any).scripts as unknown[]).forEach(validateImportScript_ACU);
  validateLibraryReferences_ACU((parsed as any).scripts as Partial<UserScriptDefinition_ACU>[], (parsed as any).libraries as Partial<UserScriptLibrary_ACU>[]);

  const existing = [...getUserScripts_ACU()];
  const existingLibraries = [...getScriptLibraries_ACU()];
  const existingNames = new Set(existing.map(script => script.name));
  const importedLibraryNames = new Set(((parsed as any).libraries as Partial<UserScriptLibrary_ACU>[]).map(library => String(library.name || '').trim()));
  const nextLibraries = existingLibraries.filter(library => !importedLibraryNames.has(library.name));
  const importedLibraries = ((parsed as any).libraries as Partial<UserScriptLibrary_ACU>[]).map(library => createStoredScriptLibrary_ACU(library, { preserveIds: false }));
  ...
  const state = ensureScriptSettings_ACU();
  state.scriptLibraries.splice(0, state.scriptLibraries.length, ...nextLibraries, ...importedLibraries);
