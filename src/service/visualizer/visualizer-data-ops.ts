...
function ensurePendingOps_ACU(state: { pendingDataOps?: PendingVisualizerDataOps_ACU | null }): PendingVisualizerDataOps_ACU {
...
export function assertVisualizerDataOpsEditable_ACU(state: { pendingDataOps?: PendingVisualizerDataOps_ACU | null }): void {
    if ((state as any)?.isSaving) {
...
interface VisualizerDraftRow_ACU {
    rowId: string;
    row: any[];
}

function getDraftSheetRows_ACU(sheet: any): { headers: any[]; rows: VisualizerDraftRow_ACU[] } {
    const content = Array.isArray(sheet?.content) ? sheet.content : [];
    const headers = Array.isArray(content[0]) ? content[0] : [];
    const rows: VisualizerDraftRow_ACU[] = [];
...
function valuesEqual_ACU(left: any, right: any): boolean {
...
export function assertVisualizerRowDataEditable_ACU(state: any): void {
...
export function assertVisualizerTemplateEditable_ACU(state: any): void {
    assertVisualizerDataOpsEditable_ACU(state);
...
export function assertVisualizerSheetDeleteEditable_ACU(state: any): void {
    assertVisualizerDataOpsEditable_ACU(state);
    if (state?.templateDirty) {
...
export function recordVisualizerDraftDataDiff_ACU(
    state: any,
    previousData: Record<string, any> | null | undefined,
    nextData: Record<string, any> | null | undefined,
    skipSheetKeys: Iterable<string> = [],
    ...
    const plans: Array<
        | { kind: 'update'; sheetKey: string; rowId: string; columnName: string; value: any }
        | { kind: 'insert'; sheetKey: string; clientRowId: string; row: any[] }
        | { kind: 'delete'; sheetKey: string; rowId: string }
    > = [];

    const sheetKeys = new Set([...Object.keys(previous), ...Object.keys(next)]);
    sheetKeys.delete('mate');
    for (const sheetKey of sheetKeys) {
        if (skipped.has(sheetKey)) continue;
        ...
        const previousById = new Map<string, VisualizerDraftRow_ACU>();
        ...
        previousSheet.headers.forEach((header, index) => {
            ...
            if (index > 0 && name && !oldHeaderIndex.has(name)) oldHeaderIndex.set(name, index);
        });
        ...
        if (schemaChanged) {
            for (const item of nextSheet.rows) {
                if (!item.rowId || candidateIdCounts.get(item.rowId) !== 1 || !previousById.has(item.rowId)) continue;
        ...
        for (const item of nextSheet.rows) {
            ...
            if (canMatch) {
                ...
                if (isTempRowId_ACU(item.rowId)) continue;
                const oldRow = previousById.get(item.rowId)!.row;
                for (let columnIndex = 1; columnIndex < nextSheet.headers.length; columnIndex += 1) {
                    ...
                    if (schemaChanged && !commonColumnNames.has(columnName)) continue;
    ...
    for (const plan of plans) {
        if (plan.kind === 'update') recordVisualizerCellUpdate_ACU(state, plan.sheetKey, plan.rowId, plan.columnName, plan.value);
        else if (plan.kind === 'insert') {
            plan.row[0] = plan.clientRowId;
            recordVisualizerRowInsert_ACU(state, plan.sheetKey, plan.clientRowId);
        } else recordVisualizerRowDelete_ACU(state, plan.sheetKey, plan.rowId);
    }
    return { templateChanged: hasTemplateChanges };
}

export function resetVisualizerPendingDataOps_ACU(state: { pendingDataOps?: PendingVisualizerDataOps_ACU | null }): void {
...
async function refreshVisualizerRuntimeFromReplay_ACU(isolationKey: string): Promise<any> {
