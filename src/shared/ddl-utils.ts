...
export interface FixedRowIdReplaceContract_ACU {
  tableName: string;
  minRowId: number;
  maxRowId: number;
}

function parseSafePositiveInteger_ACU(value: string): number | null {
...
function extractDDLCheckExpressions_ACU(definition: string): string[] {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (value.slice(index, index + 5).toUpperCase() !== 'CHECK'
      ...
      || /[A-Za-z0-9_$]/.test(value[index + 5] || '')) continue;

    let openingIndex = index + 5;
    while (/\s/.test(value[openingIndex] || '')) openingIndex += 1;
    if (value[openingIndex] !== '(') continue;
    let depth = 1;
    let expression = '';
 ...
 */
export function getFixedRowIdReplaceContract_ACU(sheet: Sheet_ACU): FixedRowIdReplaceContract_ACU | null {
  const ddl = String(sheet?.sourceData?.ddl || '');
  const tableName = parseDDLTableName(ddl);
  if (!tableName || !/^[A-Za-z_][A-Za-z0-9_$]*$/.test(tableName)) return null;

  const rowIdColumn = parseDDLColumnInfos_ACU(ddl).find(column => column.sqlName.toLowerCase() === 'row_id');
  if (!rowIdColumn?.isPrimaryKey || rowIdColumn.declaredType?.toUpperCase() !== 'INTEGER') return null;
  ...
  const constrain = (nextLower: number, nextUpper: number): boolean => {
  ...
  for (const expression of expressions) {
    ...
    if (!constrain(nextLower, nextUpper)) return null;
  }
