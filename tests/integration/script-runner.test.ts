...
vi.mock('../../src/data/repositories/profile-repo', () => ({
  ...
  ensureGlobalScriptSettings_ACU: vi.fn(() => {
    ...
    if (!Array.isArray(mocks.globalMeta.scriptLibrariesGlobal)) mocks.globalMeta.scriptLibrariesGlobal = [];
    if (!Array.isArray(mocks.globalMeta.scriptLogsGlobal)) mocks.globalMeta.scriptLogsGlobal = [];
    return { userScripts: mocks.globalMeta.userScriptsGlobal, scriptLibraries: mocks.globalMeta.scriptLibrariesGlobal, scriptLogs: mocks.globalMeta.scriptLogsGlobal };
  }),
}));
...
beforeEach(() => {
  ...
  mocks.globalMeta.scriptLibrariesGlobal = [];
  mocks.globalMeta.scriptLogsGlobal = [];
  mocks.settings.userScripts = mocks.globalMeta.userScriptsGlobal;
  mocks.settings.scriptLibraries = mocks.globalMeta.scriptLibrariesGlobal;
...
describe('ScriptRunner 阶段二能力', () => {
  ...
  it('按声明依赖加载共享脚本库并执行导出函数', async () => {
    mocks.globalMeta.scriptLibrariesGlobal.push({
      id: 'script_lib_math',
      name: 'math-lib',
      enabled: true,
      version: 1,
    ...
    addScript({
      id: 'script_uses_library',
      name: 'uses-library',
      libraryNames: ['math-lib'],
      source: 'const math = await ctx.importLibrary("math-lib"); return math.add(2, 5);',
    });

    const result = await runScriptVariable_ACU({ raw: '{[script "uses-library"]}', kind: 'execute', scriptName: 'uses-library' });

    expect(result.success).toBe(true);
    expect(result.value).toBe(7);
  });

  it('拒绝加载未声明的共享脚本库', async () => {
    mocks.globalMeta.scriptLibrariesGlobal.push({
      id: 'script_lib_secret',
      name: 'secret-lib',
      enabled: true,
      version: 1,
      language: 'javascript',
    ...
    addScript({
      id: 'script_undeclared_library',
      name: 'undeclared-library',
      source: 'return await ctx.importLibrary("secret-lib");',
    });

    const result = await runScriptVariable_ACU({ raw: '{[script "undeclared-library"]}', kind: 'execute', scriptName: 'undeclared-library' });

    expect(result.success).toBe(false);
    expect(result.error).toContain('脚本未声明脚本库依赖: secret-lib');
  });
  ...
  it('hook 脚本可调用注入的 ctx.controller，且失败时不提交 controller draft', async () => {
    addScript({
      ...
      bindings: [{ hook: 'plot.after_stage', enabled: true, order: 1, target: { presetName: '剧情预设', stage: 1 } }],
    });
    addScript({
      id: 'script_controller_fail',
      name: 'controller-fail',
      source: 'ctx.controller.skipStage(3); throw new Error("boom");',
      bindings: [{ hook: 'plot.after_stage', enabled: true, order: 2, failurePolicy: 'continue', target: { presetName: '剧情预设', stage: 1 } }],
    });
    const committedStages: number[] = [];

    const results = await runScriptHook_ACU('plot.after_stage', {
      eventPayload: { presetName: '剧情预设', stage: 1 },
      createController: () => {
        const draft: number[] = [];
        return {
