...
describe('ScriptStore 阶段一能力', () => {
  ...
  it('共享脚本库可独立创建、更新、读取和删除', () => {
    const saved = upsertScriptLibrary_ACU({
      id: 'script_lib_local',
      name: '公共库',
      enabled: true,
      version: 1,
    ...
    expect(saved.id).toBe('script_lib_local');
    expect(getScriptLibraries_ACU()).toHaveLength(1);

    upsertScriptLibrary_ACU({ ...saved, version: 2, source: 'export const value = 2;' }, false);
    expect(getScriptLibraries_ACU()[0]).toMatchObject({ name: '公共库', version: 2, source: 'export const value = 2;' });

    expect(deleteScriptLibrary_ACU(saved.id, false)).toBe(true);
    expect(getScriptLibraries_ACU()).toHaveLength(0);
  });

  it('仍被脚本引用的共享库不能删除', () => {
    const library = upsertScriptLibrary_ACU({
      id: 'script_lib_in_use',
      name: '被引用库',
      enabled: true,
      version: 1,
      language: 'javascript',
    ...
    expect(() => deleteScriptLibrary_ACU(library.id, false)).toThrow('脚本库仍被脚本引用: 被引用库 -> 引用库脚本');
  });
