...
const harness = vi.hoisted(() => ({ readConversation: vi.fn() }));

vi.mock('../../../src/service/continuation/agent/agent-conversation-store', () => ({
  readAgentConversation_ACU: harness.readConversation,
}));
...
function snapshot_ACU(messages: unknown[]): any {
  return { schemaVersion: 1, chatIdentity: 'chat-a', updatedAt: 1, messages };
}
...
describe('useContinuationSession', () => {
  it('挂载时把持久会话按消息种类投影成会话流条目', () => {
    harness.readConversation.mockReturnValue(snapshot_ACU([
      ...
      { id: 'm5', kind: 'handoff', text: '早期轮次摘要', digest: '交接报告', createdAt: 5, turnKey: null },
    ]));
  ...
  it('持久会话不可读时保持空会话流而不是让页面崩掉', () => {
    harness.readConversation.mockImplementation(() => { throw new Error('楼层不可读'); });
