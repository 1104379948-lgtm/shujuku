...
describe('rankAgentWorldbookCandidates_ACU', () => {
  ...
  it('does not create a metadata match by joining separate query fields', () => {
    const undergroundPassage = candidate('暗门', [], '地下通道入口');
    const other = candidate('酒馆', [], '酒馆休息区域');

    const ranked = rankAgentWorldbookCandidates_ACU([undergroundPassage, other], {
      userInput: '',
      recentContext: '地下',
      taskContext: '通道',
    });

    expect(ranked).toEqual([undergroundPassage, other]);
  });
