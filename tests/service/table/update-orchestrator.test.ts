...
const { mockChatArrayForSeedStage, mockIndependentTableStates, mockGetChatArray_ACU, mockCaptureManualRefillSessionSnapshot, mockClearManualRefillIncrementalDataInRange, mockClearManualRefillSheetDataInRange, mockCommitManualRefillSheetSnapshot, mockRestoreManualRefillSessionSnapshot, mockEnsureManualRefillInitialBaseline, mockEnsureBoundaryCheckpoint, mockShouldRotateBoundaryCheckpoint, mockPurgeSheetKeysFromChatHistoryHard } = vi.hoisted(() => {
  ...
  return {
    ...
    mockEnsureManualRefillInitialBaseline: vi.fn().mockResolvedValue({ success: true, changed: false, skipped: true, targetMessageIndex: 0 }),
...
beforeEach(() => {
  ...
  mockEnsureManualRefillInitialBaseline.mockResolvedValue({ success: true, changed: false, skipped: true, targetMessageIndex: 0 });
...
describe('loadBatchBaseData_ACU', () => {
  ...
  it('恢复指导表结构时保留聊天数据中的较高 nextRowId', () => {
    const chatHistory = [
      {
        is_user: false,
        TavernDB_ACU_IsolatedData: {
    ...
    loadBatchBaseData_ACU(chatHistory, 1, '', ['sheet_0'], mergedBatchData);

    expect(mergedBatchData.sheet_0.sourceData).toEqual({ note: '新说明', nextRowId: 10 });
    expect(mergedBatchData.sheet_0.content[0]).toEqual(['row_id', '新列']);
  });
...
describe('orchestrateManualUpdate_ACU', () => {
  ...
  it('可回放手动重填启动前不预清理重填范围内选中表历史数据', async () => {
  ...
  it('可回放手动重填不在启动期刷新 SQLite 运行时快照', async () => {
  ...
  it('重填起点早于最早 init checkpoint 时先用模板建立临时基底但不预清理或刷新', async () => {
    const { getChatArray_ACU, clearManualRefillIncrementalDataInRange_ACU, ensureManualRefillInitialBaseline_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue({
      mate: { type: 'acu' },
      sheet_0: { uid: 'sheet_0', name: 'chronicle', updateConfig: {}, content: [['row_id', 'code_index']] },
      sheet_1: { uid: 'sheet_1', name: 'quest_log', updateConfig: {}, content: [['row_id', 'status']] },
    });
    vi.mocked(getChatArray_ACU).mockReturnValue([
      { is_user: true },
      { is_user: false, mes: 'AI回复1' },
      ...
      { is_user: true },
      {
        ...
        TavernDB_ACU_IsolatedData: {
          '': {
            ...
            storageFrame: {
              version: 2,
              checkpoint: { kind: 'full', reason: 'init', createdAt: 1, data: { mate: { type: 'acu' }, sheet_0: { name: 'chronicle', content: [['row_id', 'code_index']] } } },
              logEntries: [],
            },
          },
        },
    ...
    mockCurrentJsonTableData = { sheet_0: { name: 'chronicle', updateConfig: {}, content: [['row_id', 'code_index'], ['old', '1']] } };
    mockEnsureManualRefillInitialBaseline.mockResolvedValueOnce({ success: true, changed: true, targetMessageIndex: 3, movedFromMessageIndex: 5, downgradedCount: 1 });
    mockCallCustomOpenAI.mockResolvedValue('<tableEdit>sheet_0</tableEdit>');

    const result = await orchestrateManualUpdate_ACU(['sheet_0'], vi.fn().mockResolvedValue({ success: true }), mockRefreshData, { clearBeforeUpdate: true });

    expect(result.success).toBe(true);
    expect(ensureManualRefillInitialBaseline_ACU).toHaveBeenCalledWith(expect.objectContaining({
      isolationKey: '',
      targetMessageIndex: 3,
      data: expect.objectContaining({ sheet_0: expect.objectContaining({ name: 'chronicle' }), sheet_1: expect.objectContaining({ name: 'quest_log' }) }),
      save: true,
    }));
    expect(mockCaptureManualRefillSessionSnapshot).toHaveBeenCalledWith([3, 5]);
    expect(mockClearManualRefillIncrementalDataInRange).not.toHaveBeenCalled();
    expect(mockReloadStorageProvider).not.toHaveBeenCalled();
  });

  it('临时 initial baseline 建立失败时回滚并拒绝清理或刷新', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
  ...
  it('临时 initial baseline 建立直接 reject 时统一回滚且返回硬失败', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    vi.mocked(getChatArray_ACU).mockReturnValue([{ is_user: false, mes: 'AI回复1' }]);
    mockCurrentJsonTableData = { sheet_0: { name: 'chronicle', updateConfig: {}, content: [['row_id', 'code_index']] } };
    mockEnsureManualRefillInitialBaseline.mockRejectedValueOnce(new Error('baseline transaction rejected'));

    const result = await orchestrateManualUpdate_ACU(['sheet_0'], vi.fn().mockResolvedValue({ success: true }), mockRefreshData, { clearBeforeUpdate: true });

    expect(result.success).toBe(false);
    expect(result.error).toContain('baseline transaction rejected');
    expect(mockRestoreManualRefillSessionSnapshot).toHaveBeenCalledTimes(1);
    expect(mockClearManualRefillIncrementalDataInRange).not.toHaveBeenCalled();
    expect(mockReloadStorageProvider).not.toHaveBeenCalled();
  });

  it('临时 baseline 落盘后 preparing 进度回调抛错时统一回滚', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    vi.mocked(getChatArray_ACU).mockReturnValue([
      { is_user: true },
      { is_user: false, mes: 'AI回复1' },
  ...
  it('可回放手动重填忽略旧的整段增量清理 mock，直接按 bucket 执行', async () => {
    const { getChatArray_ACU, clearManualRefillIncrementalDataInRange_ACU, clearManualRefillSheetDataInRange_ACU } = await import('../../../src/service/chat/chat-service');
    vi.mocked(getChatArray_ACU).mockReturnValue([
      { is_user: true },
      { is_user: false, mes: 'AI回复1' },
  ...
  it('可回放手动重填不预清理范围内或范围外历史基底，保护 V2 replay 初始数据', async () => {
    const { getChatArray_ACU, clearManualRefillIncrementalDataInRange_ACU, clearManualRefillSheetDataInRange_ACU, clearTableDataAtFloors_ACU } = await import('../../../src/service/chat/chat-service');
    const chat = [
      { is_user: true, mes: '用户0' },
      {
  ...
  it('可回放手动重填不预清理 chat[0] sheet guide，避免 V2 replay 回退链路断裂', async () => {
    const { getChatArray_ACU, clearManualRefillIncrementalDataInRange_ACU, clearManualRefillSheetDataInRange_ACU, clearTableDataAtFloors_ACU } = await import('../../../src/service/chat/chat-service');
    const chat = [
  ...
  it('无 full checkpoint 且确认前回滚失败时返回硬错误而不是确认对象', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue({
      mate: { type: 'acu' },
        ...
        TavernDB_ACU_IsolatedData: {
          '': {
            ...
            storageFrame: {
              ...
              logEntries: [{
                seq: 1,
                entryId: 'log-only-rollback-failure',
                createdAt: 1,
                source: 'auto_fill',
                targetMessageIndex: 0,
                aiFloor: 1,
    ...
    mockRestoreManualRefillSessionSnapshot.mockRejectedValueOnce(new Error('restore transaction failed'));
    const processBatch = vi.fn().mockResolvedValue({ success: true });

    const result = await orchestrateManualUpdate_ACU(['sheet_0'], processBatch, mockRefreshData, { clearBeforeUpdate: true });

    expect(result.success).toBe(false);
    expect(result.error).toContain('临时基底回滚失败：restore transaction failed');
  ...
  it('可回放手动重填跳过写入前 retained buffer boundary checkpoint，避免 bucket 提交前被维护性 checkpoint 中断', async () => {
    const { getChatArray_ACU, clearManualRefillIncrementalDataInRange_ACU, clearManualRefillSheetDataInRange_ACU } = await import('../../../src/service/chat/chat-service');
    vi.mocked(getChatArray_ACU).mockReturnValue([
      { is_user: true },
      { is_user: false, mes: 'AI回复' },
...
describe('executeCardUpdateCore_ACU — SQL 错误反馈重试', () => {
  ...
  beforeEach(async () => {
    ...
    mockCurrentJsonTableData = {
      mate: { type: 'acu', version: 1 },
      sheet_0: { uid: 'test', name: '测试表', sourceData: { ddl: 'CREATE TABLE test (row_id INTEGER PRIMARY KEY, value TEXT);' }, content: [['row_id', 'value']], updateConfig: {}, exportConfig: {}, orderNo: 0 },
    };
    mockPersistTablesToChatMessage.mockResolvedValue({ saved: true, messageIndex: 0 });
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue({
      mate: { type: 'acu', version: 1 },
      sheet_0: { uid: 'test', name: '测试表', sourceData: { ddl: 'CREATE TABLE test (row_id INTEGER PRIMARY KEY, value TEXT);' }, updateConfig: { groupId: 0 }, content: [['row_id', 'value']] },
    });
  });
  ...
  it('SQL 模式下 parseAndApplyTableEdits 抛错时，错误信息注入到 tableDataText', async () => {
    ...
    mockCallCustomOpenAI.mockImplementation(async (dynamicContent: any) => {
      callCount++;
      if (callCount === 1) {
        return '<tableEdit>UPDATE test SET value = (SELECT value FROM invalid_table) WHERE row_id = 1;</tableEdit>';
      }
      if (callCount === 2) {
        ...
        return "<tableEdit>INSERT INTO test (value) VALUES ('ok');</tableEdit>";
      }
  ...
  it('SQL 模式下多次重试时错误信息被替换（不累积）', async () => {
    ...
    mockCurrentJsonTableData = {
      mate: { type: 'acu', version: 1 },
      sheet_0: { uid: 't', name: '测试表', sourceData: { ddl: 'CREATE TABLE t (row_id INTEGER PRIMARY KEY, value TEXT);' }, content: [['row_id', 'value']], updateConfig: {}, exportConfig: {}, orderNo: 0 },
    };
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue({
      mate: { type: 'acu', version: 1 },
      sheet_0: { uid: 't', name: '测试表', sourceData: { ddl: 'CREATE TABLE t (row_id INTEGER PRIMARY KEY, value TEXT);' }, updateConfig: { groupId: 0 }, content: [['row_id', 'value']] },
    } as any);
    ...
    mockCallCustomOpenAI.mockImplementation(async (dynamicContent: any) => {
      ...
      if (callCount === 1) return '<tableEdit>UPDATE t SET value = (SELECT value FROM missing) WHERE row_id = 1;</tableEdit>';
      if (callCount === 2) return '<tableEdit>INSERT INTO t (missing_col) VALUES (1);</tableEdit>';
      return "<tableEdit>INSERT INTO t (value) VALUES ('ok');</tableEdit>";
    });
...
describe('applyUnifiedGroupFillResponses_ACU', () => {
  ...
  it('SQL 模式下基于显式 baseSnapshot 顺序统一提交，不污染全局 currentJsonTableData', async () => {
    ...
    expect(savedData.sheet_0.sourceData.nextRowId).toBe(3);
    expect(savedData.sheet_1.sourceData.nextRowId).toBe(3);
    expect(mockPersistTablesToChatMessage.mock.calls[0][0].operations).toEqual([
      { kind: 'sql_sheet_batch', sheetKey: 'sheet_0', statements: ["INSERT INTO inventory (row_id, value) VALUES (2, 'sql-a')"], tableName: 'inventory', reason: 'system' },
      { kind: 'sql_sheet_batch', sheetKey: 'sheet_1', statements: ["INSERT INTO quest_log (row_id, value) VALUES (2, 'sql-b')"], tableName: 'quest_log', reason: 'system' },
      { kind: 'meta_update', sheetKey: 'sheet_0', meta: { sourceData: { nextRowId: 3 } } },
      { kind: 'meta_update', sheetKey: 'sheet_1', meta: { sourceData: { nextRowId: 3 } } },
    ]);
    vi.mocked(isSqliteMode).mockReturnValue(false);
  });
  ...
  it('SQL batch 错误文本不含语句时仍按 preparedStatementIndex 映射到对应 group', async () => {
    const { isSqliteMode } = await import('../../../src/service/table/storage-mode');
    ...
    const baseSnapshot = {
      mate: { type: 'acu', version: 1 },
      sheet_0: {
        uid: 'inventory', name: '表A',
        sourceData: { ddl: 'CREATE TABLE inventory (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);' },
        content: [['row_id', 'value'], ['1', 'base-a']], updateConfig: {}, exportConfig: {}, orderNo: 0,
      },
      sheet_1: {
        uid: 'quest_log', name: '表B',
        sourceData: { ddl: 'CREATE TABLE quest_log (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);' },
        content: [['row_id', 'value'], ['1', 'base-b']], updateConfig: {}, exportConfig: {}, orderNo: 1,
      },
    } as any;
    mockCurrentJsonTableData = JSON.parse(JSON.stringify(baseSnapshot));
    disposeStorageProvider();
    const provider = await ensureStorageProviderReady_ACU();
    const batchSpy = vi.spyOn(provider, 'applyEditsBatchWithSheetMetadata').mockReturnValue({
      success: false,
      appliedEdits: 0,
      modifiedKeys: [],
      error: 'constraint failed',
      batchPhase: 'prepared_statement',
      preparedStatementIndex: 1,
    } as any);
    const responses = [
      { success: true, attempt: 1, aiResponse: "<tableEdit>UPDATE inventory SET value = 'a2' WHERE row_id = 1;</tableEdit>", tableEditText: "UPDATE inventory SET value = 'a2' WHERE row_id = 1;", job: { groupKey: 'a', groupId: 1, batchNumber: 1, saveTargetIndex: 3, targetSheetKeys: ['sheet_0'], updateMode: 'auto_standard', requestOptions: null, messagesForContext: [], baseSnapshot, isImportMode: false } },
      { success: true, attempt: 1, aiResponse: "<tableEdit>UPDATE quest_log SET value = 'b2' WHERE row_id = 1;</tableEdit>", tableEditText: "UPDATE quest_log SET value = 'b2' WHERE row_id = 1;", job: { groupKey: 'b', groupId: 2, batchNumber: 1, saveTargetIndex: 3, targetSheetKeys: ['sheet_1'], updateMode: 'auto_standard', requestOptions: null, messagesForContext: [], baseSnapshot, isImportMode: false } },
    ];

    const result = await applyUnifiedGroupFillResponses_ACU(responses as any, baseSnapshot, { saveTargetIndex: 3, updateMode: 'auto_standard', isImportMode: false });

    expect(result.success).toBe(false);
    expect(result.error).toContain('group b SQL 执行失败');
  ...
  it('SQL 受控提交不从外部 seedRows 重建 canonical 空表，replay 与 runtime 深一致', async () => {
    ...
    const { loadTableStateFromFramesV2_ACU } = await import('../../../src/service/table/storage-frame-v2-replay');
    vi.mocked(isSqliteMode).mockReturnValue(true);

    const ddl = 'CREATE TABLE inventory (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);';
    const baseSnapshot = {
      mate: { type: 'acu', version: 1 },
      sheet_0: {
        uid: 'inventory',
        name: '表A',
        sourceData: { ddl, nextRowId: 3 },
        content: [['row_id', 'value'], ['1', 'old-a'], ['2', 'old-b']],
        updateConfig: {},
        exportConfig: {},
        orderNo: 0,
      },
    ...
    disposeStorageProvider();
    const provider = await ensureStorageProviderReady_ACU();
    provider.applyEdits('DELETE FROM inventory;');
    expect(provider.getCurrentData()?.sheet_0.content).toEqual([['row_id', 'value']]);
    vi.mocked(getEffectiveSeedRowsForSheet_ACU).mockReturnValue([
      ['1', 'seed-a'],
      ['2', 'seed-b'],
    ] as any);

    const runtimeBase = JSON.parse(JSON.stringify(provider.getCurrentData()));
    const responses = [{
      success: true,
      attempt: 1,
      aiResponse: "<tableEdit>UPDATE inventory SET value = 'updated' WHERE value = 'seed-a';</tableEdit>",
      tableEditText: "UPDATE inventory SET value = 'updated' WHERE value = 'seed-a';",
      job: {
        groupKey: 'a', groupId: 1, batchNumber: 1, saveTargetIndex: 3,
        targetSheetKeys: ['sheet_0'], updateMode: 'auto_standard', requestOptions: null,
        messagesForContext: [], baseSnapshot: runtimeBase, isImportMode: false,
      },
    }];

    const result = await applyUnifiedGroupFillResponses_ACU(
      responses as any,
      runtimeBase,
    ...
    const replayChat = [{
      ...
      TavernDB_ACU_IsolatedData: {
        '': {
          ...
          storageFrame: {
            version: 2,
            checkpoint: { kind: 'full', createdAt: 1, reason: 'init', data: runtimeBase, event: { filledSheetKeys: [], changedSheetKeys: [], groupKeys: [] } },
            logEntries: [{ seq: 1, entryId: 'reseed-ai-update', createdAt: 2, source: 'group_fill', targetMessageIndex: 0, aiFloor: 1, filledSheetKeys: ['sheet_0'], changedSheetKeys: ['sheet_0'], groupKeys: ['a'], operations: savePayload.operations }],
          },
        },
      },
    }];
  ...
  it('SQL 受控提交不因外部 seedRows 生成非目标表 operation', async () => {
    ...
    const baseSnapshot = {
      ...
      sheet_0: {
        uid: 'inventory', name: '表A',
        sourceData: { ddl: 'CREATE TABLE inventory (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);', nextRowId: 2 },
        content: [['row_id', 'value'], ['1', 'old-a']], updateConfig: {}, exportConfig: {}, orderNo: 0,
      },
      sheet_1: {
        uid: 'quest_log', name: '表B',
        sourceData: { ddl: 'CREATE TABLE quest_log (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);', nextRowId: 2 },
        content: [['row_id', 'value'], ['1', 'old-b']], updateConfig: {}, exportConfig: {}, orderNo: 1,
      },
    } as any;
    mockCurrentJsonTableData = JSON.parse(JSON.stringify(baseSnapshot));
    ...
    disposeStorageProvider();
    const provider = await ensureStorageProviderReady_ACU();
    provider.applyEdits('DELETE FROM inventory; DELETE FROM quest_log;');
    vi.mocked(getEffectiveSeedRowsForSheet_ACU).mockImplementation((sheetKey: string) => (
      sheetKey === 'sheet_0' ? [['1', 'seed-a']] as any : [['1', 'seed-b']] as any
    ));
    const runtimeBase = JSON.parse(JSON.stringify(provider.getCurrentData()));
    const beforeRuntime = JSON.parse(JSON.stringify(runtimeBase));
    const responses = [{
      success: true,
      attempt: 1,
      aiResponse: "<tableEdit>UPDATE inventory SET value = 'updated' WHERE value = 'seed-a';</tableEdit>",
  ...
  it('SQL 受控提交无法导出 canonical data 时明确中止，不回退 workingData', async () => {
    const { isSqliteMode } = await import('../../../src/service/table/storage-mode');
    const { disposeStorageProvider, ensureStorageProviderReady_ACU } = await import('../../../src/service/table/table-storage-strategy');
    vi.mocked(isSqliteMode).mockReturnValue(true);
    ...
    disposeStorageProvider();
    const provider = await ensureStorageProviderReady_ACU();
    const exportCanonicalDataSpy = vi.spyOn(provider, 'exportCanonicalData').mockImplementation(() => { throw new Error('canonical export rejected'); });
    const responses = [{
      success: true,
      attempt: 1,
      aiResponse: "<tableEdit>UPDATE inventory SET value = 'updated' WHERE row_id = 1;</tableEdit>",
      tableEditText: "UPDATE inventory SET value = 'updated' WHERE row_id = 1;",
      job: {
        groupKey: 'a', groupId: 1, batchNumber: 1, saveTargetIndex: 3,
        targetSheetKeys: ['sheet_0'], updateMode: 'auto_standard', requestOptions: null,
        messagesForContext: [], baseSnapshot, isImportMode: false,
      },
    }];
    ...
    expect(result.success).toBe(false);
    expect(result.error).toContain('无法导出 SQLite canonical data');
    expect(mockPersistTablesToChatMessage).not.toHaveBeenCalled();
    exportCanonicalDataSpy.mockRestore();
    expect(provider.executeQuery('SELECT value FROM inventory WHERE row_id = 1').values).toEqual([['base-a']]);
  ...
  it('SQL 模式下缺失表的外部 seedRows 不会写入 canonical snapshot', async () => {
    ...
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue({
      mate: { type: 'acu', version: 1 },
      sheet_0: { uid: 'inventory', name: '表A', sourceData: { ddl: 'CREATE TABLE inventory (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);' }, content: [['row_id', 'value'], ['1', 'tpl-a']], updateConfig: {}, exportConfig: {}, orderNo: 0 },
      sheet_1: { uid: 'quest_log', name: '表B', sourceData: { ddl: 'CREATE TABLE quest_log (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);', nextRowId: 10 }, content: [['row_id', 'value']], updateConfig: {}, exportConfig: {}, orderNo: 1 },
    } as any);
...
describe('processGroupedRuntimeChunk_ACU', () => {
  ...
  it('手动重填多 bucket 首次使用安全边界，后续 bucket 继承本轮已提交基底且隔离旧未来帧', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    ...
    expect(promptBaseRows[1]).toHaveLength(29);
    expect(promptBaseRows[1].some(row => row[0] === '27' || row[0] === '28')).toBe(true);
    expect(promptBaseRows[1].some(row => row[0] === '29' || row[0] === '30')).toBe(false);
  });
  ...
  it('SQL grouped 提交会在执行前以安全基底对齐 provider，未来行不会影响 row_id 或持久化快照', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    const { isSqliteMode } = await import('../../../src/service/table/storage-mode');
    const { disposeStorageProvider, ensureStorageProviderReady_ACU } = await import('../../../src/service/table/table-storage-strategy');
    ...
    const baseSnapshot = {
      ...
      sheet_0: {
        uid: 'inventory', name: '表A',
        sourceData: { ddl: 'CREATE TABLE inventory (row_id INTEGER PRIMARY KEY, value TEXT NOT NULL);', nextRowId: 2 },
        content: [['row_id', 'value'], ['1', 'safe-history']], updateConfig: {}, exportConfig: {}, orderNo: 0,
      },
    } as any;
    vi.mocked(getChatArray_ACU).mockReturnValue([{ is_user: true }, { is_user: false, mes: 'AI回复' }]);
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue(baseSnapshot);
    mockCurrentJsonTableData = JSON.parse(JSON.stringify(baseSnapshot));
    disposeStorageProvider();
    const provider = await ensureStorageProviderReady_ACU();
    provider.applyEdits("INSERT INTO inventory (row_id, value) VALUES (99, 'future-old');");
    const replaceAllDataSpy = vi.spyOn(provider, 'replaceAllData');
    const batchSpy = vi.spyOn(provider, 'applyEditsBatchWithSheetMetadata');
    let persistedData: any = null;
    mockPersistTablesToChatMessage.mockImplementation(async (options: any) => {
      persistedData = JSON.parse(JSON.stringify(options.tableData));
      return { saved: true, messageIndex: options.targetMessageIndex };
    });
    mockCallCustomOpenAI.mockResolvedValueOnce("<tableEdit>INSERT INTO inventory (value) VALUES ('committed');</tableEdit>");
    ...
    try {
      ...
      expect(replaceAllDataSpy).toHaveBeenCalledTimes(1);
      expect(batchSpy).toHaveBeenCalledTimes(1);
      expect(provider.executeQuery('SELECT row_id, value FROM inventory ORDER BY row_id').values).toEqual([
        [1, 'safe-history'],
        [2, 'committed'],
      ]);
      expect(provider.exportCanonicalData().sheet_0.content).toEqual([
        ['row_id', 'value'],
        ['1', 'safe-history'],
        ['2', 'committed'],
      ]);
      expect(persistedData.sheet_0.content).toEqual([
        ['row_id', 'value'],
        ['1', 'safe-history'],
        ['2', 'committed'],
      ]);
    } finally {
      replaceAllDataSpy.mockRestore();
      batchSpy.mockRestore();
      disposeStorageProvider();
      vi.mocked(isSqliteMode).mockReturnValue(false);
    }
  ...
  it('SQL provider 安全基底对齐失败时不执行 SQL、不落盘且不启动后续 bucket', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    const { isSqliteMode } = await import('../../../src/service/table/storage-mode');
    const { disposeStorageProvider, ensureStorageProviderReady_ACU } = await import('../../../src/service/table/table-storage-strategy');
    ...
    vi.mocked(getChatArray_ACU).mockReturnValue([
      { is_user: true }, { is_user: false, mes: 'AI回复1' },
      { is_user: true }, { is_user: false, mes: 'AI回复2' },
    ]);
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue(baseSnapshot);
    mockCurrentJsonTableData = JSON.parse(JSON.stringify(baseSnapshot));
    disposeStorageProvider();
    const provider = await ensureStorageProviderReady_ACU();
    provider.applyEdits("INSERT INTO inventory (row_id, value) VALUES (99, 'future-old');");
    const replaceAllDataSpy = vi.spyOn(provider, 'replaceAllData').mockResolvedValue({
      success: false, appliedEdits: 0, modifiedKeys: [], error: 'forced replacement failure',
    });
    const batchSpy = vi.spyOn(provider, 'applyEditsBatchWithSheetMetadata');
    const executionState: { workingSnapshot?: Record<string, any> } = {};
    mockCallCustomOpenAI
      .mockResolvedValueOnce("<tableEdit>INSERT INTO inventory (value) VALUES ('first');</tableEdit>")
      .mockResolvedValueOnce("<tableEdit>INSERT INTO inventory (value) VALUES ('second');</tableEdit>");

    try {
      const result = await processGroupedRuntimeChunk_ACU([
        { key: 'first_bucket', groupId: 0, indices: [1], batchSize: 1, sheetKeys: ['sheet_0'], requestOptions: null },
        { key: 'second_bucket', groupId: 0, indices: [3], batchSize: 1, sheetKeys: ['sheet_0'], requestOptions: null },
      ], 'manual_independent', { executionState });

      expect(result.success).toBe(false);
      expect(result.error).toContain('SQLite runtime 无法对齐到当前 bucket 安全基底');
      expect(result.failedGroups).toContain('first_bucket');
      expect(mockCallCustomOpenAI).toHaveBeenCalledTimes(1);
      expect(replaceAllDataSpy).toHaveBeenCalledTimes(1);
      expect(batchSpy).not.toHaveBeenCalled();
      expect(mockPersistTablesToChatMessage).not.toHaveBeenCalled();
      expect(executionState.workingSnapshot?.sheet_0.content).toEqual([
        ['row_id', 'value'],
        ['1', 'safe-history'],
      ]);
      expect(provider.executeQuery('SELECT row_id, value FROM inventory ORDER BY row_id').values).toEqual([
  ...
  it('SQL provider 替换后 canonical 仍不一致时恢复运行时并停止后续 bucket', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    const { isSqliteMode } = await import('../../../src/service/table/storage-mode');
    const { disposeStorageProvider, ensureStorageProviderReady_ACU } = await import('../../../src/service/table/table-storage-strategy');
    ...
    const provider = await ensureStorageProviderReady_ACU();
    provider.applyEdits("INSERT INTO inventory (row_id, value) VALUES (99, 'future-old');");
    const originalReplaceAllData = provider.replaceAllData.bind(provider);
    const replaceAllDataSpy = vi.spyOn(provider, 'replaceAllData').mockImplementation(async replacement => {
      const result = await originalReplaceAllData(replacement);
      provider.applyEdits("INSERT INTO inventory (row_id, value) VALUES (99, 'forced-desync');");
      return result;
    });
    const batchSpy = vi.spyOn(provider, 'applyEditsBatchWithSheetMetadata');
    const executionState: { workingSnapshot?: Record<string, any> } = {};
    ...
    try {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('严格对齐后校验失败');
      expect(mockCallCustomOpenAI).toHaveBeenCalledTimes(1);
      expect(replaceAllDataSpy).toHaveBeenCalledTimes(1);
      expect(batchSpy).not.toHaveBeenCalled();
      expect(mockPersistTablesToChatMessage).not.toHaveBeenCalled();
  ...
  it('SQL 已提交后的 readable lorebook 同步失败不会阻断 workingSnapshot 推进', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    const { isSqliteMode } = await import('../../../src/service/table/storage-mode');
    const { disposeStorageProvider } = await import('../../../src/service/table/table-storage-strategy');
    ...
    vi.mocked(getChatArray_ACU).mockReturnValue([{ is_user: true }, { is_user: false, mes: 'AI回复' }]);
    vi.mocked(parseTableTemplateJson_ACU).mockReturnValue(snapshot);
    mockCurrentJsonTableData = JSON.parse(JSON.stringify(snapshot));
    mockUpdateReadableLorebookEntry.mockRejectedValueOnce(new Error('readable sync unavailable'));
    mockCallCustomOpenAI.mockResolvedValueOnce("<tableEdit>INSERT INTO inventory (value) VALUES ('committed');</tableEdit>");
    const executionState: { workingSnapshot?: Record<string, any> } = {};
  ...
  it('Native 已提交后的派生同步失败不会重试已提交 bucket，后续 bucket 继承 workingSnapshot', async () => {
    const { getChatArray_ACU } = await import('../../../src/service/chat/chat-service');
    const { parseTableTemplateJson_ACU } = await import('../../../src/shared/utils');
    vi.mocked(getChatArray_ACU).mockReturnValue([
      { is_user: true }, { is_user: false, mes: 'AI回复1' },
    ...
    mockUpdateReadableLorebookEntry.mockRejectedValueOnce(new Error('readable sync unavailable'));
    mockEnqueueSummaryVectorIndexFlush.mockRejectedValueOnce(new Error('vector sync unavailable'));
    mockCallCustomOpenAI
      .mockResolvedValueOnce('<tableEdit>sheet_0</tableEdit>')
      .mockResolvedValueOnce('<tableEdit>sheet_0</tableEdit>');
    const executionState: { workingSnapshot?: Record<string, any> } = {};
    ...
    expect(mockCallCustomOpenAI).toHaveBeenCalledTimes(2);
    expect(mockPersistTablesToChatMessage).toHaveBeenCalledTimes(2);
    expect(mockUpdateReadableLorebookEntry).toHaveBeenCalledTimes(2);
    expect(mockEnqueueSummaryVectorIndexFlush).toHaveBeenCalledTimes(2);
    expect(promptBaseRows).toEqual([
      [['row_id', '值'], ['1', 'base-a']],
      [['row_id', '值'], ['1', 'base-a'], ['2', '来自A']],
    ]);
    expect(executionState.workingSnapshot?.sheet_0.content).toEqual([
      ['row_id', '值'], ['1', 'base-a'], ['2', '来自A'], ['2', '来自A'],
    ]);
  });
  ...
  it('统一提交失败后会把错误反馈注入下一轮 prompt 并重试成功', async () => {
    ...
    mockParseAndApplyTableEditsToData.mockImplementation((_aiResponse: string, tableData: any) => {
      tableData.sheet_0.content.push(['attempted', '未提交候选数据']);
      return { success: true, modifiedKeys: ['sheet_0'], appliedEdits: 1 };
    });
    ...
    expect(promptBaseRows).toHaveLength(2);
    expect(promptBaseRows[0]).toEqual([['row_id', '值'], ['1', 'base-a']]);
    expect(promptBaseRows[1]).toEqual([['row_id', '值'], ['1', 'base-a']]);
    expect(mockPersistTablesToChatMessage).toHaveBeenCalledTimes(2);
  });
