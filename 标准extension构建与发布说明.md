...
2. 再把产物发布到直装仓库 `OWNER/shujuku-extension-only`
...
```text
https://github.com/OWNER/shujuku-extension-only
```
...
```powershell
git clone --branch extension-only --single-branch https://github.com/OWNER/shujuku-extension-only.git $PublishDir
```
...
git -C $PublishDir `
  ...
  -c user.email="release-bot@example.com" `
...
```powershell
gh api repos/OWNER/shujuku-extension-only/commits/extension-only
```
...
```powershell
gh api repos/OWNER/shujuku-extension-only/contents?ref=extension-only
```
...
# 6. 提交并推送
git -C $PublishDir add index.js manifest.json
git -C $PublishDir -c user.name="Release Bot" -c user.email="release-bot@example.com" commit -m "build: refresh extension-only from ${SourceSha}-dirty" -m "Source-Ref: standard dist/extension from working-tree over $SourceSha"
git -C $PublishDir push origin extension-only
```
